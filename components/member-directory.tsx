  ...
  return <Toaster><div className="members-page mt-4 space-y-4">
    ...
    <section className="members-table-card">
      ...
      <div className="members-table-scroll"><table className="data-table member-table"><thead><tr><th><MemberCheckbox label="Select page members" checked={allPageSelected} onChange={(checked) => setSelected(checked ? [...new Set([...selected, ...pageRows.map(member => member.id)])] : selected.filter(id => !pageRows.some(member => member.id === id)))} /></th>{['Photo', 'Name', 'ID Number', 'Role / Team', 'Type', 'Status', 'Validity', 'Action'].map(label => <th key={label}>{label}</th>)}</tr></thead>
        <tbody>{pageRows.map(member => { const isSelected = selected.includes(member.id); return <tr key={member.id} data-selected={isSelected || undefined}><td><MemberCheckbox label={`Select ${displayName(member)}`} checked={isSelected} onChange={(checked) => setSelected(checked ? [...new Set([...selected, member.id])] : selected.filter(id => id !== member.id))} /></td><td><span className="member-photo-fallback" title={member.photo_path ? 'Photo available' : 'No photo uploaded'}>{displayName(member).split(/\s+/).map(part => part[0]).join('').slice(0, 2).toUpperCase()}</span></td><td className="member-name-cell"><p>{displayName(member)}</p><small>{member.tip_email}</small></td><td className="member-id-cell">{member.aws_sbg_id}</td><td className="member-role-cell">{member.officer_position || member.team || member.program || member.membership_type}</td><td>{member.membership_type}</td><td><StatusBadge status={member.status} /></td><td className="member-validity-cell">{member.valid_until ?? 'Not issued'}</td><td><div className="member-row-actions"><Link className="btn" href={`/generate-id?member=${member.id}`}>Review / Edit ID</Link><button className="btn" aria-label={`Archive ${displayName(member)}`} disabled={!!busy} onClick={() => { setModalError(''); setArchiveTarget(member); }}>Archive</button></div></td></tr>; })}</tbody>
      </table></div>
