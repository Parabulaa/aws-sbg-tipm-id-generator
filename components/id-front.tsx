...
export function IDFront({ member }: IdFrontProps) {
  return (
    <div className="relative flex h-full flex-col overflow-hidden bg-[#fffaf0] text-slate-950">
      <div className="absolute right-0 top-0 h-48 w-28 bg-amber-400" />
      ...
      <div className="absolute -left-20 bottom-20 size-48 rotate-45 rounded-[32px] border-[26px] border-amber-400/60" />
      ...
      <main className="relative z-10 flex flex-1 flex-col items-center px-7 pt-6 text-center">
        <div className="grid aspect-[4/5] w-[68%] place-items-center overflow-hidden rounded-[22px] border-4 border-white bg-gradient-to-br from-amber-100 to-amber-300 text-5xl font-black text-amber-800 shadow-lg shadow-amber-900/10">
          {member.fullName.split(' ').slice(0, 2).map((name) => name[0]).join('')}
        </div>
        <p className="mt-6 text-[10px] font-bold uppercase tracking-[0.2em] text-amber-700">Member profile</p>
        <h3 className="mt-2 text-xl font-black leading-tight tracking-tight">{member.fullName}</h3>
        <p className="mt-2 text-sm font-bold text-amber-700">Member</p>
